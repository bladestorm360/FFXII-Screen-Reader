...
| `F10` | **Sneak assist — On ⇄ Off (Session 106).** Speaks the new setting. Same setting the mod menu holds. **Default OFF**, and it acts ONLY on maps the danger table names (`path_danger.cpp`; map 568 today) — everywhere else it is inert whichever way it is set. On those maps it stops the guards' proximity check from catching you. It writes no game state: switching it off restores the game's own behaviour on the very next check. See `sneak_assist.h` | free — game binds F1/F2/F3 only |
