...
> | Soundscape | **Off** (default) / On | **Session 191.** **Everything** interactable within about **20 steps** sounds, continuously, from the direction it is in — exits, doors, shops, save and gate crystals, chests, people, enemies and dropped items all sound different. There is no limit on how many: the 20 steps is the only limit. Each thing repeats on **its own clock**, so the sound follows it as it moves and as you move. Nearer things are louder; things behind you are quieter, duller and lower, exactly as the beacons already do. Where several of a kind are near you they are **spread in pitch, up to 30% either side of normal** — the first is normal, the next 5% higher, the next 5% lower, then 10% higher, 10% lower, and so on — and each repeats at a slightly different rate (about a second, then 1.1 seconds, then 1.2…). The pitch tells them apart; the different spacing is what keeps them from landing on top of each other. A sound always finishes before it repeats, so the longer ones space themselves out a little further. It speaks nothing and it is not a route: `\` and the beacon are unchanged. It goes quiet in conversations, in the battle command menu, and while the field is not yours to drive. **Floor traps have no sound and are left out** — there is no trap sound to use, and borrowing another one would be telling you something false. |
> | Soundscape volume | 20% / 40% / 60% / 80% / **100%** (default) | How loud the soundscape plays. |
>
