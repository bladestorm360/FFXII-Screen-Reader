...
uint64_t __fastcall HookedFieldFrame() {
    ...
    FrameProbe::OnFieldFrame();
    {
        ...
        // Stilshrine of Miriam statue capture. Off that dungeon it is five slot reads
        // and a return -- it arms on a live `mrm_` script, never on a map id.
        { STALL_SCOPE("StatueDiag::OnFieldFrame"); StatueDiag::OnFieldFrame(); }
